...
class Ido_REST_Faxians_Controller extends WP_REST_Posts_Controller{

	protected $post_type;
	protected $meta;
	public function __construct($post_type='faxian'){
	...
	public function get_items($request){
        ...
        $idocalgary_query_where = function($where, $wp_query) use ($request){
        	if(!empty($request['dist'])){
		        ...
		        $lon1 = $user_lng - $dist / (cos(deg2rad($user_lat)) * 69);
		        $lon2 = $user_lng + $dist / (cos(deg2rad($user_lat)) * 69);
